...
from aiogram.types import FSInputFile  # Измените импорт на FSInputFile
from config import *
from utils import *
...
async def fetch_news(first_run=False):
    async with async_playwright() as p:
        browser = await p.chromium.launch(headless=False)  # Оставьте headless=False для отладки
        context = await browser.new_context()  # Создаем контекст для браузера

        # Обходим каждый сайт в списке
        for site in news_sites:
            page = await context.new_page()  # Открываем новую страницу в контексте
            print(f"Открываем сайт: {site}")  # Отладочное сообщение

            try:
                await page.goto(site, timeout=60000)  # Тайм-аут 60 секунд
                print(f"Сайт {site} загружен успешно.")  # Отладочное сообщение
...
async def process_treknews(page, first_run=False):
    """Функция для парсинга сайта treknews.net"""
...
async def send_news_from_db(first_run=False):
    ...
    if first_run:
        ...
        # Отправляем только те, которые еще не отправлены (sent = 0)
